...
func can_play(context: Dictionary) -> bool:
	if not super.can_play(context):
		return false
	# If no target selected yet, affordability alone is enough to enter targeting.
	...
	if not target_hexes.is_empty():
		...
		if board:
			...
			return tile.occupant is Creature
	return true
...
func get_valid_targets(board: HexGrid) -> Array[Vector2i]:
	if board == null:
		return []
	# Valid targets are hexes with friendly creatures on them.
