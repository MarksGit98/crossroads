...
## Resolve the card's declared effects. Delegates to Creature's static
## dispatcher, which owns all state-mutation logic (cards are data; creatures
## are the agents that apply damage, status, etc.).
func resolve_effects(ctx: DuelContext) -> void:
	Creature.apply_effect_list(card_data.effects, ctx)
