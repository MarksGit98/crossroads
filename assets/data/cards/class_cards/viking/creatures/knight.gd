## Knight — Viking balanced melee striker.
## Well-rounded fighter with decent stats across the board.
static func data() -> CardData:
	var card := CardData.new()

	# Identity
	card.id = "viking_knight"
	card.card_name = "Knight"
	card.card_type = CardTypes.CardType.CREATURE
	card.card_class = CardTypes.Class.VIKING
	card.rarity = CardTypes.Rarity.COMMON
	card.flavor = "Honor is the shield no blade can shatter."

	# Cost
	card.cost_type = CardTypes.CostType.MANA
	card.cost_value = 3

	# Stats
	card.role = CardTypes.CreatureRole.STRIKER
	card.atk = 4
	card.hp = 5
	card.armor = 1
	card.move_range = 2
	card.move_pattern = CardTypes.MovePattern.STANDARD
	card.attack_range = 1
	card.attack_pattern = CardTypes.AttackPattern.MELEE
	card.damage_type = CardTypes.DamageType.PHYSICAL

	# Keywords
	card.keywords = [CardTypes.Keyword.VIGILANT]

	# Creature scene
	card.creature_scene_path = "res://scenes/creatures/viking/knight.tscn"

	return card
